...
@property (retain, nonatomic) PingDataAggregator *pingDataAggregator;
@property (retain, nonatomic) NSMutableArray *dataPoints;
...
- (void)dealloc {
    [networkGraphView release];
    [runTestButton release];
    [averagePingLabel release];
    [minimumPingLabel release];
    [maximumPingLabel release];
    
    //[self.dataPoints release];
    //[self.pingDataAggregator release];
    [signalQualityTitleLabel release];
    [graphScaleAdjustmentSlider release];
    [graphScaleLabel release];
    [signalQualityBackgroundImageView release];
    [hostSelectionButton release];
    [pingVarianceLabel release];
    [pingThresholdLabel release];
    [pingVarianceStepper release];
    [pingThresholdStepper release];
    [super dealloc];
}
...
- (void)setupAggregator{
    ...
    self.pingDataAggregator = [[[PingDataAggregator alloc]initWithHostName:host numberOfPings:self.numberOfTestPings packetSizeBytes:0 pingVarianceMilliseconds:pingVariance pingVarianceThreshold:pingThreshold]autorelease];
...
- (IBAction)touchSelectHost:(id)sender {
    ...
    UIAlertView *selectHostAlert = [[[UIAlertView alloc]initWithTitle:@"Select Host" message:message delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"OK",nil]autorelease];
...
- (void)graphData:(NSDictionary*)dataPoint{
    ...
    float ave = [[self.pingDataAggregator pingAverage]floatValue];
    
    if (ave < 1) {
        viewHeight = responseTime * 95;
    }
    else if (ave <= 5){
        viewHeight = responseTime * 15;
    }
    else if (ave <= 10){
        viewHeight = responseTime * 6;
    }
    else if (ave <= 19){
        viewHeight = responseTime * 3.5;
    }
    else if (ave <= 38){
        viewHeight = responseTime * 1.5;
    }
    else if (ave <= 57){
        viewHeight = responseTime;
    }
    else if (ave <= 76){
        viewHeight = responseTime * .75f;
    }
    else if (ave <= 95){
        viewHeight = responseTime * .60f;
    }
    else if (ave <= 114){
        viewHeight = responseTime * .70f;
    }
    else if (ave <= 133){
        viewHeight = responseTime * .43f;
    }
    else if (ave <= 152){
        viewHeight = responseTime * .35f;
    }
    else if (ave <= 171){
        viewHeight = responseTime * .33f;
    }
    else if (ave <= 190){
        viewHeight = responseTime * .30f;
    }
    else {
        viewHeight = responseTime * .25f;
    }
    
    UIView *graphView = [[UIView alloc]initWithFrame:CGRectMake(_graphCurrentX, 189.0f, 4.5f, -viewHeight)];
