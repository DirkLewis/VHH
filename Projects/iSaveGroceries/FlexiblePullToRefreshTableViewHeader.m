...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [VHHStyleSheet styleFlexibleHeaderView:self];

		UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0.0f, frame.size.height - 30.0f, self.frame.size.width, 20.0f)];
        [VHHStyleSheet styleFlexibleHeaderLastUpdatedLabel:label];
		[self addSubview:label];
		_lastUpdatedLabel=label;
		
		label = [[UILabel alloc] initWithFrame:CGRectMake(0.0f, frame.size.height - 48.0f, self.frame.size.width, 20.0f)];
        [VHHStyleSheet styleFlexibleHeaderStatusLabel:label];
		[self addSubview:label];
		_statusLabel=label;
		
        CALayer *layer = [CALayer layer];
        [VHHStyleSheet styleFlexibleHeaderImageLayer:layer frame:frame];
